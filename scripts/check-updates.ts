#!/usr/bin/env tsx
/**
 * Zambian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://zambialii.org';
const PORTAL_NAME = 'Zambia Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/zambian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Zambian Law MCP — Data Freshness Check');
