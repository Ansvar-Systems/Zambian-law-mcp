#!/usr/bin/env tsx
/**
 * Database builder for Zambia Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Zambia Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Zambian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'ZM');
    insertMeta.run('source', 'zambialii.org');
