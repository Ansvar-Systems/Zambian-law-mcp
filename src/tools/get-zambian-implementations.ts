/**
 * get_zambian_implementations — Find Zambian statutes implementing a specific EU directive/regulation
...
export interface GetZambianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface ZambianImplementationResult {
  document_id: string;
...
export async function getZambianImplementations(
  db: InstanceType<typeof Database>,
  input: GetZambianImplementationsInput,
): Promise<ToolResponse<ZambianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as ZambianImplementationResult[];
