/**
 * search_eu_implementations — Search EU directives/regulations that have Zambian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_zambian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as zambian_statute_count
  ...
  if (input.has_zambian_implementation) {
    sql += ' HAVING zambian_statute_count > 0';
  }
